...
class BST
{
	public:
		...
		void transplant(Node * node_to_replace, Node * new_node)
		{
...
class RBT
{
	public:
		...
		RBNode * insert2(string word)
		{
			...
			RBNode * leftGuard = new RBNode();
			RBNode * rightGuard = new RBNode();
			leftGuard -> setKey("");
			leftGuard -> setParent(newNode);
			leftGuard -> setLeft(NULL);
			leftGuard -> setRight(NULL);
			leftGuard -> setColor(0);
			rightGuard -> setKey("");		
			rightGuard -> setParent(newNode);
			rightGuard -> setLeft(NULL);
			rightGuard -> setRight(NULL);
			rightGuard -> setColor(0);

			newNode -> setKey(word);
			newNode -> setParent(NULL);
			newNode -> setLeft(leftGuard);
			newNode -> setRight(rightGuard);
		...
		void rbDelFixup(RBNode * node)
		{
			while(node != root && node -> color == 0)
			{
				if(node == ((RBNode *)node -> parent -> left))
				{
					...
					if(((RBNode *)w -> left) -> color == 0 && ((RBNode *)w -> right) -> color == 0)
					{
						w -> setColor(1);
				...
				else
				{
					...
					if(w -> color == 1)
					{
						w -> setColor(0);
						((RBNode *)node -> parent) -> setColor(1);
						rightRotate(((RBNode *)node -> parent));
						w = ((RBNode *)node -> parent -> left);
					}
					if(((RBNode *)w -> right) -> color == 0 && ((RBNode *)w -> left) -> color == 0)
					{
						w -> setColor(1);
						node = ((RBNode *)node -> parent);
					}
...
public:
	...
	BloomFilter(int hashes, int filterSize)
	{
		hashesNumber = hashes;
		size = filterSize;
		array = new bool[size];
	...
	int hash_function(string value, int i)
	{
		size_t str_hash = hash<string>{}(value);
		return (str_hash * (i + 1)) % size;
	}

	void insert(string value)
	{
		for(int i = 0; i < hashesNumber; i++)
		{
			if(array[hash_function(value, i)] == false)
				setBits++;
			array[hash_function(value, i)] = true;
		}
	}
...
int main(int argc, char * argv[])
{
	...
	if(strcmp(argv[2], "bst") == 0)
		...
		bloomFilter = new BloomFilter(4, 128);
	else
	{
	...
	for(int i = 0; i < n; i++)
	{
		...
		else if(instruction == "find")
		{
			...
			{
				int result = bloomFilter -> contains(param);
				if(result == 0)
					cout << "0" << endl;
				else
					cout << "1" << endl;
	...
	{
		cerr << "insert: " << rbt -> insert_counter << endl;
		cerr << "delete: " << rbt -> delete_counter << endl;
		cerr << "find: " << rbt -> search_counter << endl;
		cerr << "min: " << rbt -> min_counter << endl;
		cerr << "max: " << rbt -> max_counter << endl;
		cerr << "succesor: " << rbt -> succesor_counter << endl;
		cerr << "in order: " << rbt -> inOrder_counter << endl;
		cerr << "max fill: " << rbt -> max_fill << endl;
		cerr << "elements at end: " << rbt -> number_of_el << endl;
	}
	else if(hashTable != NULL)
	{
