...
int RandomSelect(int A[], int p, int q, int k, int * comp, int * shifts)
{
	...
	int r = RandomPartition(A, p, q, comp, shifts);
	cerr << "pivot: " << r << endl;
	int s = r - p + 1;
	...
	if(k == s)
		return A[r];
	if(k < s)
		RandomSelect(A, p, r, k, comp, shifts);
	else
...
int RandomPartition(int A[], int p, int r, int * comp, int * shifts)
{
	int i = rand() % r + p;
	swap(A[r - 1], A[i]);
	return partition(A, p, r, comp, shifts);
}
...
int Select(int A[], int p, int q, int k, int * comp, int * shifts)
{
	...
	{
		insertionSort(A, p + i * 5, p + (i + 1) * 5, comp, shifts);
		medians[j] = A[p + 2 + i * 5];
		j++;
	}
	...
	int x = Select(medians, 0, n, (n + 1) / 2, comp, shifts);
	int y = selectPartition(A, p, q - 1, x);

	if(y == k - 1)
		return x;
	if(k - 1 < y)
		return Select(A, p, y, k, comp, shifts);
	else
